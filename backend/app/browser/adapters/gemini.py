...
class GeminiAdapter(BaseAdapter):
    """Google Gemini 网页版适配器"""

    platform_name = "gemini"
    platform_url = "https://gemini.google.com"

    # DOM 选择器
    selector_input = '.ql-editor[contenteditable="true"], div[contenteditable="true"][aria-label*="prompt"], .input-area [contenteditable]'
    selector_send_btn = 'button[aria-label="Send message"], .send-button, button[data-testid="send-button"]'
    selector_response = '.model-response-text, .response-container .markdown, [data-content="prose"]'
    selector_response_text = '.model-response-text, .response-container .markdown'
    selector_stop_btn = 'button[aria-label="Stop"], .stop-button'
    selector_login_form = 'a[href*="accounts.google.com"], button[aria-label*="Sign in"]'
    selector_captcha = ''
    selector_new_chat = 'button[aria-label="New chat"], a[aria-label="New chat"]'
    selector_error_toast = '.error-toast, [role="alert"]'
