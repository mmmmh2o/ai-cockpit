...
@AdapterRegistry.register("lmarena")
class LMArenaAdapter(BaseAdapter):
    """LMArena (lmarena.ai) 网页版适配器"""

    platform_name = "lmarena"
    platform_url = "https://lmarena.ai"

    selector_input = 'textarea, [contenteditable="true"]'
    selector_send_btn = 'button[type="submit"], button:has-text("Send")'
    selector_response = '.message-content, [class*="response"], [class*="bot-message"]'
    selector_response_text = '.message-content, [class*="response"]'
    selector_stop_btn = 'button:has-text("Stop"), [class*="stop"]'
    selector_login_form = 'a[href*="login"], button:has-text("Login")'
    selector_captcha = ''
    selector_new_chat = 'button:has-text("New"), a:has-text("New")'
    selector_error_toast = '[class*="toast"], [class*="error"]'
